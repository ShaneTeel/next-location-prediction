...
class LocationGenerator:
    ...
    def _initialize_model(self, distance, min_k):
        ...
        logger.info(f"Initialized DBSCAN with epsilon == {epsilon}")
    ...
    def generate_locations(self, stay_points:pd.DataFrame):
        ...
        filtered = self._filter(stay_points, labels)
        ...
        return pd.merge(
               ...
               how='inner', 
               on='cluster'
               ).rename(columns={'lat': 'precise_lat', 'lon': 'precise_lon'}).sort_values(by="arrived")
    ...
    def _filter(self, stay_points, labels):
    ...
    def _calculate_centroids(self, filtered_stay_points):
    ...
    def _evaluate(self, labels, stay_points):
